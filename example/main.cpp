...
{
    void observe(const VesselState &s) override
    {
        std::cout << s.at("H") << std::endl;
    }
};

// struct GraphReactionVisitor : stochastic::ReactionVisitor
// {
//     std::map<std::string, size_t> symbols;
//     GraphReactionVisitor() : symbols() {}

//     void visit(const Reaction *r)
//     {
//     }

//     void visit(const Species *s)
//     {
//         symbols[s->label] = s
//     }
// };
...
int main()
{
    HospitalizationObserver observer{};

    auto model = seihr(100000);
